...
async function startImport() {
  ...
  for (let i = 0; i < rows.value.length; i++) {
    ...
    try {
      const data = await props.config.createRow(buildPayload(row))
      ...
      if (props.config.onRowCreated) {
        ...
        await props.config.onRowCreated(data, row.extra).catch((err) => {
          warning = err instanceof Error ? err.message : "Un effet secondaire de cette ligne a échoué."
        })
      }
      results.value.push({ index: i, success: true, warning })
    } catch (err) {
      results.value.push({ index: i, success: false, error: err instanceof Error ? err.message : 'Échec de la création' })
    }
